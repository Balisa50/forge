...
export async function GET(req: NextRequest) {
  ...
  if (!m) return NextResponse.json({ gated: false });
  const week = parseInt(m[1], 10);

  const entry = CURATED_ROADMAPS.find((r) => r.title === roadmapTitle);
  if (!entry) return NextResponse.json({ gated: false });
  const slug = entry.slug;

  const curriculum = loadRoadmap(slug);
  if (!curriculum) return NextResponse.json({ gated: false });
  const wk = curriculum.weeks.find((w) => w.number === week);
  if (!wk || !wk.days || wk.days.length === 0) return NextResponse.json({ gated: false });

  const requiredKeys: string[] = [];
  for (const d of wk.days) {
    d.items.forEach((_, i) => requiredKeys.push(`d${d.number}-i${i}`));
  }
  if (requiredKeys.length === 0) return NextResponse.json({ gated: false });
